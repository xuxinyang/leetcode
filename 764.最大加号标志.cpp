...
public:
    int orderOfLargestPlusSign(int n, vector<vector<int>> &a)
    {
        ...
        down.clear(n);
        for (int i = 1; i <= n; ++i)
            tmp[i].clear(), col[i].clear(), col[i].push_back(0);
        for (int i = 0; i < m; ++i)
            tmp[++a[i][0]].push_back(++a[i][1]);
        for (int i = 1; i <= n; ++i)
            for (int x : tmp[i])
                col[x].push_back(i);
        for (int i = 1; i <= n; ++i)
            tmp[i].clear(), row[i].clear(), row[i].push_back(0);
        for (int i = 0; i < m; ++i)
            tmp[a[i][1]].push_back(a[i][0]);
        for (int i = 1; i <= n; ++i)
            for (int x : tmp[i])
                row[x].push_back(i);

        for (int i = 1; i <= n; ++i)
            info[i].clear(), row[i].push_back(n + 1), col[i].push_back(n + 1);
        for (int i = 1; i <= n; ++i)
        {
